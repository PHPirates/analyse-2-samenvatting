...
\begin{document}
	...
	\subsection{Stellingen uit het huiswerk}
	\theorem Zij $D \subset \reals^d$. Een functie $f:D \rightarrow \reals^m$ heet \indx{Lipschitz continu} als er een $L>0$ is zodanig dat
	...
	\theorem[Schwarz of Clairaut] Neem $D \subset \reals^d$ open. Als $f:D \rightarrow \reals$ twee keer continu partieel d'baar in $D$
 ...
 er is een lineaire afbeelding $L_{\a}\colon\reals^d \mapsto \reals$ bestaat zodanig dat
	\[f(\a+\h)=f(\a)+ L_{\a} \h +\mathrm{o}(\abs{\h})\iff\lim_{\h\to0}\frac{\abs{f(\a+\h)-f(\a)-L_{\a} \h}}{\abs{h}} = 0\]
 dan heet $f\colon D \mapsto \reals$ \indx{differenti\"eerbaar} (\indx{totaal differenti\"eerbaar}, \indx{Fr\`echet' differenti\"eerbaar}) te $\a \in D$
...
\]
	...
	\theorem Zij $D \subset \reals^d$, $D$ open. Als $f\colon D \rightarrow \reals$ d'baar te $\a \in D$ dan is $f$ ook continu te $\a$.
	...
	\newpage	
	\section{College 21}
		Zij $D \subset \reals^d$ open, $\a\in D$, $f\colon D \rightarrow \reals$.
...
%\]
		...
		\end{tabular}
		\theorem[K 10.4.5] Zij $D\subset\reals^d$, $f\colon D\mapsto\reals$ part d'baar in een omgeving van $\a \in \intr D$ en $\partial_i f \quad i=1,\dots ,d$ continu te $\a$. Dan $f$ d'baar in $\a$.
...
\begin{define}[Richtingsafgeleide]
			Zij $D \subset \reals^d$ open, $\a \in D$, $\u \in \reals^d$ een richtingsvector. Zij $f\colon D \rightarrow \reals$ d'baar te $a$.
Dan is de \indx{richtingsafgeleide} van $f$ in de richting van $\u$ gegeven door
			\begin{equation} \label{eq:rafg}
			   	  		D_{\u} f(\a) := \lim_{t \rightarrow 0} \dfrac{f(\a+t\u) - f(\a)}{t}
			\end{equation}
			Speciale gevallen:
...
Verder kan met behulp van bovenstaande definities aangetoond worden dat:
		...
		\begin{theorem}[\text{[K]} 11.1.10]
			Zij $f\colon R \subset \reals^2 \to \reals$ continu op een rechthoek $R = [a, b] \times [c, d]$.
		...
		\begin{theorem}[\text{[K] 11.2.7a}]
			Zij $f\colon [a, b] \to \reals$ en $g\colon [c, d] \to \reals$ beide Riemannintegreerbaar. Zij verder gegeven de rechthoek $R = [a, b] \times [c, d]$. Dan
			\[ 
